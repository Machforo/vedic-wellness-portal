...
export default function HerbalGardenPage() {
  ...
  const herbalGarden = data?.herbalGarden || {};
  const defaultDesc = `The IAMC Herbal Garden is one of the most distinctive features of the campus — over 200 medicinal plant species cultivated as the living classroom for Dravyaguna Vigyana students, research scholars, and visiting Ayurvedic practitioners. Plants are labelled with Sanskrit, common, and botanical names.

BAMS students use the garden for plant identification exercises, collection of specimens for classical drug preparation practicals in the Rasa Shastra laboratory, and understanding the habitat and seasonal availability of medicinal plants.

Guided tours are arranged for incoming BAMS batches, visiting scholars, and delegates at the Global Ayurvedic Summit. The garden represents the living connection between ancient text and present-day clinical practice.`;
  
  const plants = herbalGarden.plants?.length > 0 ? herbalGarden.plants : [
          ...
          <div className="grid lg:grid-cols-2 gap-12 mb-16">
            ...
            <div className="reveal-right">
              <div className="rounded-2xl overflow-hidden shadow-2xl border bg-navy/5">
                <img src={herbalGarden.image || "https://images.unsplash.com/photo-1416879595882-3373a0480b5b?q=80&w=800&auto=format&fit=crop"} alt="IAMC Herbal Garden" className="w-full h-[400px] object-cover" />
              </div>
            </div>
          </div>
              ...
              <table className="w-full text-sm border-collapse shadow-sm">
                <thead>
                  <tr className="bg-navy text-white">
                    <th className="text-left p-4 font-bold rounded-tl-lg">Sanskrit Name</th>
                    <th className="text-left p-4 font-bold">Botanical Name</th>
                    <th className="text-left p-4 font-bold">Part Used</th>
                    <th className="text-left p-4 font-bold rounded-tr-lg">Therapeutic Use</th>
                  </tr>
                </thead>
                <tbody>
                  {plants.map((p: any, i: number) => (
                    <tr key={i} className={i % 2 === 0 ? "bg-muted/30" : "bg-white"}>
                      <td className="p-4 font-semibold text-gold border-b">{p.sanskrit}</td>
                      <td className="p-4 italic text-foreground/70 border-b">{p.latin}</td>
                      <td className="p-4 text-foreground/70 border-b">{p.part}</td>
                      <td className="p-4 text-foreground/70 border-b">{p.use}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
