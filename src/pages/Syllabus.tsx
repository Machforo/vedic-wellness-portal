...
export default function SyllabusPage() {
  ...
  const phases = syllabus.phases && syllabus.phases.length > 0 ? syllabus.phases : [
        ...
        <div className="container-wide max-w-5xl">
          ...
          <div className="space-y-6">
            {phases.map((p: any, i: number) => {
              const cleanSubjects = p.subjects && typeof p.subjects === 'string' ? p.subjects.replace(/<\/?[^>]+(>|$)/g, "") : p.subjects || "";
              const subjectList = cleanSubjects ? cleanSubjects.split(',').map((s: string) => s.trim()) : [];
              return (
                <div key={i} className={`reveal delay- border rounded-2xl bg-card overflow-hidden`}>
                  <div className="flex items-center justify-between p-6 bg-navy/[0.03] border-b">
                    ...
                    {p.pdf && p.pdf !== "#" && (
                      <a href={p.pdf} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 px-4 py-2 bg-gold/10 text-navy text-xs font-bold rounded-lg hover:bg-gold/20 transition-colors">
                  ...
                  <div className="p-6">
                    <div className="flex flex-wrap gap-2">
                      {subjectList.map((s: string, idx: number) => (
                        <span key={idx} className="flex items-center gap-1.5 px-3 py-1.5 bg-muted rounded-lg text-xs font-medium">
